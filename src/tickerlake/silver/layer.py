...
class SilverLayer:
    ...
    def process_incremental_data(
        ...
        # For volume ratios, keep just enough history to maintain the 20-day window
        ...
        # Backfill extra history for tickers that still lack enough rows
        ...
        if existing_daily_data.height > 0:
            combined_parts.insert(
                0, existing_daily_data.sort(["ticker", "date"])
            )
